...
# Reverse proxy uses this URL (Guacamole default is http://localhost:17649/guacamole/):
GUAC_URL=http://localhost:17649/guacamole/
...
GUAC_URL_REDIR=""               # Auto redirect of host root URL http://xxx:17649 to http://xxx:17649/guacamole  (true/false)
...
CERT_ORG="roggerwilliam"            # Self signed cert setup, must not be blank
...
# Prompt to redirect http://root:17649 to http://root:17649/guacamole if not installing reverse proxy
if [[ -z ${GUAC_URL_REDIR} ]] && [[ "${INSTALL_NGINX}" = false ]]; then
    echo -e -n "FRONT END: Redirect Guacamole http://domain.root:17649 to /guacamole [y/n]? [default y]: "
    read PROMPT
    if [[ ${PROMPT} =~ ^[Nn]$ ]]; then
        GUAC_URL_REDIR=false
    else
...
if [[ $? -ne 0 ]]; then
    ...
    echo -e "${LGREEN}Guacamole install complete\nhttp://${PROXY_SITE}:17649 - login user/pass: guacadmin/guacadmin\n${LYELLOW}***Be sure to change the password***${GREY}"
else
    echo -e "${LGREEN}Guacamole install complete\nhttp://${PROXY_SITE}:17649/guacamole - login user/pass: guacadmin/guacadmin\n${LYELLOW}***Be sure to change the password***${GREY}"
fi
